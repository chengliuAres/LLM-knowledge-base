...
description: >-
  ...
  时先用 search 做语义发现 → 再 Read 工具补全上下文。
...
```
⚠️ 注意：search 极少返回真 0 条，更常见的是返回"假阳性"
...
**AI 整理答案**（从结果里挑出对人类友好的字段）：

> 找到 1 个最相关结果（`ghmail` 仓库 iOS 项目）：
> - **`GHMailBoxSelectVC`** — `DevPods/GHComponents/Classes/Mail/GHMailBoxSelectVC.m:32-44`（ObjC interface，score=0.84）
>
...
> 没找到 `urs_web_login_page`（拼错了，应该是 `.dart`）。换搜索角度试试：
> 1. `search --query "urs_web_login 登录网页"` 找到真实文件名
> 2. 如果搜出来有多个同名文件，从 results 里挑一个再 Read
...
```bash
python3 scripts/kb_rest.py search --query "MailBox 邮件列表 ViewController" --top_k 3
```
...
```bash
python3 scripts/kb_rest.py trace --symbol AccountMocker --direction hierarchy --depth 2
```
...
> - `chain.nodes/edges` 是多跳 BFS 完整图，深度 1-3
